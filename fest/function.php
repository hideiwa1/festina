...
function dbconnect(){
    $dsn = 'mysql:dbname=festina; host=localhost; charset=utf8';
    $user = 'root';
    $password = 'root';
    $options = array(
...
function getDiaryAll($str, $category, $span, $minlist, $flg = true){
    try{
        ...
        $sql ='SELECT b.id, b.title, b.comment, b.user_id, b.update_date, u.name, u.pic FROM blog AS b LEFT JOIN users AS u ON b.user_id = u.id WHERE b.title LIKE :key AND b.delete_flg = 0';
        if(!empty($category)){
            $sql .= ' AND category = :c_id';
            $data = array(':key' => $str, ':c_id' => $category);
        }else{
...
function getEventAll($str, $category, $span, $minlist, $flg = true){
    try{
        ...
        $sql ='SELECT e.id, e.title, e.body, e.start_date, e.end_time, e.limit_date, e.user_id, e.update_date, u.name, u.pic FROM event AS e LEFT JOIN users AS u ON e.user_id = u.id WHERE e.title LIKE :key AND e.delete_flg = 0';
        if(!empty($category)){
            $sql .= ' AND category = :c_id';
            $data = array(':key' => $str, ':c_id' => $category);
        }else{
